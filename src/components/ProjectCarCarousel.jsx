...
export default function ProjectCarCarousel({ projects, language, onSelect }) {
  ...
  const onPointerDown = (e) => {
    ...
    // very often starts on top of this card (it's a large target), and
    // grabbing pointer capture immediately fought the browser's native
    // vertical scroll instead of letting it through (same root cause the
    // skills marquee had). Wait for confirmed horizontal intent first.
    dragRef.current = {
      tracking: true,
      confirmed: false,
      startX: e.clientX,
